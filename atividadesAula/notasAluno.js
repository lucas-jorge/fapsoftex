...
while (true) {
  ...
  switch (opcao) {
    case "1":
      ...
      if (nota1 <= 10 && nota2 <= 10 && !isNaN(nota1) && !isNaN(nota2) && typeof nome === 'string') {
...
function media(notas) {
  return (Math.round(notas[0]) + Math.round(notas[1])) / 2;
}

// função para retornar se o aluno passou por média.
function status(studentMedia) {
  if (studentMedia >= 7) {
    console.log("Aluno aprovado.");
  } else if (studentMedia >= 5 && studentMedia < 7) {
    console.log("Aluno em recuperação.");
  } else if (studentMedia < 5) {
